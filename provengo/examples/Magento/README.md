...
⚠️ NOTE: In the below listings, we assume that `provengo` is in the system's PATH variable, and that `D:\Yeshayahu\Java\Magento\Examples` is the path to this directory.
...
Perform a single run through the specification. Good for "Sanity checks", i.e. to see examples of what can happen.
    ...
    provengo run --dry D:\Yeshayahu\Java\Magento\Examples
...
Draw the specification in a PDF file.
    ...
    provengo analyze -f pdf D:\Yeshayahu\Java\Magento\Examples
...
Sample 10 scenarios into a file. The scenarios are stored in a file called `samples.json` (this can be changed using the `-o`/`--output-file` switch).
    ...
    provengo sample --delete-previous --size 10 D:\Yeshayahu\Java\Magento\Examples
...
**Requires running `sample` first** (the previous command)**.**
    ...
    provengo ensemble --size 5 D:\Yeshayahu\Java\Magento\Examples
...
Draw the specification, and highlight the traces in the optimized test suite create by the previous command.
    ...
    provengo analyze -f pdf --highlight ensemble.json D:\Yeshayahu\Java\Magento\Examples

### Create Test Scripts for Third Party Systems

Converts the runs in `ensemble.json` to automation test scripts.

    provengo gen-scripts -s ensemble.json D:\Yeshayahu\Java\Magento\Examples
